...
class IngredientData {
	...
	public function add(){
		$sql = "insert into ".self::$tablename." (code,name,price_in,price_out,user_id,proveedor_id,unit) ";
		$sql .= "value (\"$this->code\",\"$this->name\",\"$this->price_in\",\"$this->price_out\",$this->user_id,\"$this->proveedor_id\",\"$this->unit\")";
...
// partiendo de que ya tenemos creado un objecto IngredientData previamente utilizamos el contexto
	...
	public function updatei($id){
		$sql = "update ".self::$tablename." set name=\"$this->name\",price_in=\"$this->price_in\",
		price_out=\"$this->price_out\",unit=\"$this->unit\",proveedor_id=\"$this->proveedor_id\" where id= $id";
		Executor::doit($sql);
	}
